...
unit leastsquares;
interface
uses Math, Ap, Sysutils, spline3, lq, bidiagonal, bdsvd, blas, svd;
...
function CalculateChebyshevLeastSquares(const M : Integer;
...
begin
    ...
    begin
        ...
        while I<=NI do
        begin
            if D[I]>MachineEpsilon*10*Sqrt(NI)*D[1] then
            begin
                B2[1,I] := B2[1,I]/D[I];
            end
            else
...
begin
    ...
    begin
        ...
        if (WS[1]=0) or (WS[NC]<=MachineEpsilon*10*Sqrt(NC)*WS[1]) then
        begin
            Result := False;
            Exit;
        end;
    ...
    while I<=ReducedSize do
    begin
        if (WS[I]<>0) and (WS[I]>MachineEpsilon*10*Sqrt(NC)*WS[1]) then
        begin
            Tmp[I] := Tmp[I]/WS[I];
        end
        else
