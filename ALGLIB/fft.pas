...
begin
    ...
    if N mod 2=0 then
    begin
        H[Floor(N/2)] := F[Floor(N/2)].X;
    end
    else
    begin
        H[Floor(N/2)] := F[Floor(N/2)].X-F[Floor(N/2)].Y;
        H[Floor(N/2)+1] := F[Floor(N/2)].X+F[Floor(N/2)].Y;
    end;
    FFTR1D(H, N, FH);
    I:=0;
    while I<=N-1 do
